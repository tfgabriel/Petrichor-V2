...
class AbsEncoder(name: String, val rev: Boolean, off: Double, var rmv: Double, val enabled: Boolean):
    Encoder<AnalogInput> {
    override val enc: AnalogInput? = if(enabled) RobotOb.hardwareMap.get(AnalogInput::class.java, name) else null
